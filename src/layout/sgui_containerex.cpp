...
bool SContainerEx::hasTextStyle() const {
    return m_hasTextColor || m_hasTextContent || 
           m_fontSize != 14.0f || m_fontFamily != SGUI_DEFAULT_FONT_FAMILY ||
...
void SContainerEx::drawBackgroundCairo(cairo_t* cr, float x, float y, float width, float height) {
    ...
    if (m_hasBackgroundGradient) {
        ...
        for (const auto& stop : m_backgroundGradient.stops) {
            cairo_pattern_add_color_stop_rgba(gradient, stop.position, 
        ...
        // 纯色背景
        cairo_set_source_rgba(cr, m_backgroundColor.r, m_backgroundColor.g, 
...
void SContainerEx::drawBorderCairo(cairo_t* cr, float x, float y, float width, float height) {
    ...
    // 设置边框颜色
    if (m_hasBorderColor) {
        cairo_set_source_rgba(cr, m_borderColor.r, m_borderColor.g, 
    ...
    if (m_hasBorderRadius) {
        ...
        if (radius > 0) {
            ...
            cairo_arc(cr, borderX + borderWidth - radius, borderY + radius, 
                     radius, -M_PI/2, 0);
            cairo_line_to(cr, borderX + borderWidth, borderY + borderHeight - radius);
            cairo_arc(cr, borderX + borderWidth - radius, borderY + borderHeight - radius, 
                     radius, 0, M_PI/2);
            cairo_line_to(cr, borderX + radius, borderY + borderHeight);
            cairo_arc(cr, borderX + radius, borderY + borderHeight - radius, 
                     radius, M_PI/2, M_PI);
            cairo_line_to(cr, borderX, borderY + radius);
            cairo_arc(cr, borderX + radius, borderY + radius, 
                     radius, M_PI, -M_PI/2);
            cairo_close_path(cr);
        } else {
...
void SContainerEx::drawTextCairo(cairo_t* cr, float x, float y, float width, float height) {
    if (!m_hasTextContent || m_text.empty()) return;
    ...
    // 设置文本颜色
    if (m_hasTextColor) {
        cairo_set_source_rgba(cr, m_textColor.r, m_textColor.g, 
    ...
    // 设置字体
    cairo_select_font_face(cr, m_fontFamily.c_str(), 
                          (m_fontStyle == FontStyle::Italic) ? CAIRO_FONT_SLANT_ITALIC : CAIRO_FONT_SLANT_NORMAL,
